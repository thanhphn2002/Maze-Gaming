...
#define SOUTH 2 // duoi
#define WEST 3 // trai
#define Toado pair <int, int> // gộp 2 đối tượng hàng và cột thành 1 tọa độ
#define H first // H = hàng
#define C second // C = cột
...
class Mecung // đọc và giải mê cung
{
	int n, m;
	int A[100][100];

public:
	void Mecung::Path() // Set điểm đầu và đích
	{
		Toado s, f;
		s.H = s.C = 2;
		f.C = GRID_WIDTH - 1;
	...
	void Mecung::TRY(Toado *x, int k, Toado f) // Tìm đường đi tối ưu
	{
		int hh[4] = { 0, -1, 0, 1 };
		int hc[4] = { 1, 0, -1, 0 };
		if (x[k] == f) Print(x, k);
	...
	void Mecung::Print(Toado *x, int k) // ghi tọa độ đường đi trong mảng 2 chiều
	{
		cout << "\n";
		FOR(i, 1, k)
		{
	...
	void Mecung::LoadFile(char *fn) // Nhận file mê cung từ máy
	{
		ifstream f(fn);
		f >> n >> m;
		FOR(i, 1, n)
...
void showTime()
{
	// hiện thời gian 
	clock_t end = clock();
	temp_time = double(end - start) / 1000;
	gotoxy(92, 11);
	TextColor(15);
	cout << temp_time << " s";
...
//Toa do trong mang 1 chieu
int XYToIndex(int x, int y)
{
	return y * GRID_WIDTH + x;
}

// kiem tra toa do trong gioi han me cung
int IsInBounds(int x, int y)
{
	...
	while (x != GRID_WIDTH - 2 && y != GRID_HEIGHT - 2)
	{
		while (lap)
		{
			...
			case 75:    // trai
				...
				break;
			case 13: // auto move khi đang giải
				autoMove();
				loser();
				break;
			case 27:	// thoat
